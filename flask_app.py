...
-	RATE_LIMIT_DAYS = 7  # 1 download a settimana
-	
-	app = Flask(__name__)
-	
...
-	    return time.time()
-	
-	def epoch_to_readable(e: float) -> str:
-	    try:
...
-	    """Somma Haversine tra le locations Valhalla [{'lat':..,'lon':..}], chiude ad anello se roundtrip."""
...
-	
-	    # Chiudi l’anello se roundtrip (Valhalla: aggiungi start alla fine)
-	    if roundtrip:
-	        start = locations[0]
...
-	            return jsonify(ok=True)
-	
-	        # Access control: permetti solo a owner/autorizzati di proseguire oltre annulla/ricomincia
-	        if uid != OWNER_ID and uid not in AUTHORIZED:
-	            # Non autorizzato → richiedi accesso se non già pendente
...
-	                if now_utc_epoch() - last < RATE_LIMIT_DAYS * 86400:
-	                    when = epoch_to_readable(last + RATE_LIMIT_DAYS * 86400)
...
-	
-	            # Pre-check limiti prima della chiamata a Valhalla
-	            if approx_km > MAX_ROUTE_KM * 1.25:  # cuscinetto per approssimazione
...
-	
-	            # Aggiorna rate limit
-	            LAST_DOWNLOAD[uid] = now_utc_epoch()
-	
-	            reset_state(uid)
-	            return jsonify(ok=True)
-	
...
-	        return jsonify(ok=True)
-	
-	    # Access control: blocca tutto il resto ai non autorizzati
-	    if uid != OWNER_ID and uid not in AUTHORIZED:
